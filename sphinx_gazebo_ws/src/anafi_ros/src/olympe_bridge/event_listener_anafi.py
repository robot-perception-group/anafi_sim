...
class EventListenerAnafi(olympe.EventListener):
	def __init__(self, drone):
		...
		self.drone.pub_position = rospy.Publisher("drone/location_slow", NavSatFix, queue_size=1)
	...
	def onGpsLocationChanged(self, event, scheduler):
		...
		self.msg_gps_location.position_covariance[0] = (math.sqrt(gps_location['latitude_accuracy']) if gps_location['latitude_accuracy'] > 0 else float('nan'))
		self.msg_gps_location.position_covariance[4] = (math.sqrt(gps_location['longitude_accuracy']) if gps_location['longitude_accuracy'] > 0 else float('nan'))
		self.msg_gps_location.position_covariance[8] = (math.sqrt(gps_location['altitude_accuracy']) if gps_location['altitude_accuracy'] > 0 else float('nan'))
		
		self.msg_gps_location.position_covariance_type = 2
	...
	def onPositionChanged(self, event, scheduler):
		...
		"altitue": event.args["altitude"],
		}
		# print("published position",position_data)
		self.msg_position.header.stamp = rospy.Time.now()
		self.msg_position.header.frame_id = "/world"
		self.msg_position.latitude = event.args["latitude"]
		self.msg_position.longitude = event.args["longitude"]
		self.msg_position.altitude = event.args["altitude"]
		self.drone.pub_position.publish(self.msg_position)
