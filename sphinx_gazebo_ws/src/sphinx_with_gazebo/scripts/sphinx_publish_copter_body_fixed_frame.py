...
node_name = 'sphinx_body_fixed_frame_publisher_node'
drone_name = rospy.get_param(rospy.get_namespace()+node_name+'/drone_name')

#Odometry topic of the drone
sphinx_topic = ("/"+drone_name+"/sphinx/drone_data",Sphinx)
...
def publish_body_fixed_frame_as_tf_frame(parent_frame,br,phi,theta,psi,x,y,z):
...
def read_sphinx_msg(msg): 
    ...
    publish_body_fixed_frame_as_tf_frame('world',br, phi, theta, psi, x,y,z)
    return

sphinx_subscriber = rospy.Subscriber(sphinx_topic[0],sphinx_topic[1],read_sphinx_msg)
rospy.loginfo("publisher node for body fixed  frame started ")
rospy.spin()
