...
class MPCOSQPWaypoint():
    ...
    def set_mpc_parameters(self):
        ...
        self.vmax =  np.array([10,10,2]) #x,y,z - velocity limits, z-component is used for MPC input
                               ...
                               #       x         y        z         vx     vy     vz              
        self.weights_Q  =   np.array([ 0e-5,     0e-5,   0e-5,    1e0,   1e0,   1e0])
        self.weights_QN =   np.array([ 1.0e3,    1.0e3,  1.0e3,   5e1,   5e1,   5e1])
        ...
        #Init variables
        self.xend = np.zeros(6)
        self.xbegin = np.ones(6)
        return
    ...
    def set_script_parameters(self):
        ...
        #Current state
        self.x_state = np.zeros(6) # in stability axes, x_state = [x_ref,x,y_ref,y,vx_ref,vx,vy_ref,vy_x_eint.y_eint,vx_eint,vy_eint]
    ...
    def set_up_mpc_problem(self):
        ...
        self.Ad = sparse.csc_matrix(np.block([
            #Update of states x
            [np.eye(3), self.dt*np.eye(3)], #update of positional states
            [np.zeros((3,3)),np.eye(3)],    #update of velocity states
        ]))
        ...
        #                         x      y        z     vx           vy             vz      
        self.xmin = np.array([-np.inf,-np.inf,-np.inf,self.vmin[0],self.vmin[1],self.vmin[2]])
        self.xmax = np.array([ np.inf, np.inf, np.inf,self.vmax[0],self.vmax[1],self.vmax[2]])
        ...
        print("np.shape(self.Ad)",self.Ad)
        # - linear dynamics
    ...
    def update_x0(self):
        ...
        x0 = self.x_state
        self.x0 = x0
        #Make sure that x0 is inside value range of allowed velocities. Positional states are not constrained (i.e. value range allows +-inf)
        self.x0[3] = np.clip(self.x0[3],self.xmin[3],self.xmax[3])
        self.x0[4] = np.clip(self.x0[4],self.xmin[4],self.xmax[4])
        self.x0[5] = np.clip(self.x0[5],self.xmin[5],self.xmax[5])
        return
    ...
    def update_mpc_problem(self):
        ...
        xr = np.ones((6,self.N+1))

        #Tracked position is updated with constant velocity model over time horizon
        xr[0,:] = self.x_desired[0] + np.arange(self.N+1)*self.dt*self.x_desired[3]
        xr[1,:] = self.x_desired[1] + np.arange(self.N+1)*self.dt*self.x_desired[4]
        xr[2,:] = self.x_desired[2] + np.arange(self.N+1)*self.dt*self.x_desired[5]
        xr[3,:] = self.x_desired[3]
        xr[4,:] = self.x_desired[4]
        xr[5,:] = self.x_desired[5]
    ...
    def read_uav_state(self,msg):
        self.x_state[0]  =  msg.pose.pose.position.x
        self.x_state[1]  =  msg.pose.pose.position.y
        self.x_state[2]  =  msg.pose.pose.position.z
        self.x_state[3]  =  msg.twist.twist.linear.vector.x
        self.x_state[4]  =  msg.twist.twist.linear.vector.y
        self.x_state[5] =  msg.twist.twist.linear.vector.z
    ...
    def read_waypoint(self,msg):
        ...
        self.x_desired[1] = self.waypoint[1]
        self.x_desired[2] = self.waypoint[2]
        self.x_desired[3] = self.waypoint[3]
        self.x_desired[4] = self.waypoint[4]
        self.x_desired[5] = self.waypoint[5]
    ...
    def publish_rpyt(self):
        ...
        # print("v.vector.z - vt.vector.z",v.vector.z - vt.vector.z)
...
if __name__ == '__main__':
    ...
    while not rospy.is_shutdown():
        ...
        if solve_successfull: 
            ...
            print("\033[93mCould not find solution\033[0m")
            debug.update_mpc_problem()
